...
function mostrarDatos(valor, pagina, cantidad) {
	...
	$.ajax({
			url: base_url+"admin/cursos_ajax",
			type: "POST",
			data: {buscar:valor, numPagina:pagina, cantidad:cantidad},
			dataType: "json",

			success:function(respuesta){
				var registros = eval(respuesta.cursos);
				...
				for (var i = registros.length - 1; i >= 0; i--) {
					html+="<tr><td> "+registros[i]["tipo"]+"</td>";
                    html+="<td> "+registros[i]["nombre"]+"</td>";
					html+="<td> "+registros[i]["sede"]+"</td>";
					html+="<td> "+registros[i]["horas"]+"</td>";
					html+="<td> "+registros[i]["fecha"]+"</td>";
					html+="<td> "+registros[i]["instructor"]+"</td>";
					html+=""+
						"<td class='text-right'>"+
                        "<button type='button' class='btn btn-icon-toggle btn-conceptos' value="+registros[i]["id_curso"]+" data-toggle='modal' data-target='#formModalConceptos'>"+
                        "<i class='fa fa-list-alt' data-toggle='tooltip' data-placement='top' data-original-title='Agregar conceptos'></i>"+
                        "</button>"+
                        	"<button type='button' class='btn btn-icon-toggle btn-lista' value="+registros[i]["id_curso"]+" data-toggle='modal' data-target='#listaModal'>"+
                                    "<i class='fa fa-users' data-toggle='tooltip' data-placement='top' data-original-title='Lista de participantes'></i>"+
                                    "</button>"+
                            "<button type='button' class='btn btn-icon-toggle btn-edit' data-toggle='modal' data-target='#formModalEdit' value="+registros[i]["id_curso"]+" data-original-title='Editar'>"+
                            "<i class='fa fa-pencil'></i></button>"+
                            "<button type='button' class='btn btn-icon-toggle btn-delete' data-toggle='tooltip' data-placement='top' value="+registros[i]["id_curso"]+" data-original-title='Borar'>"+
