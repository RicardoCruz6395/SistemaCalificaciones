...
function mostrarDatos(valor, pagina, cantidad) {
	...
	$.ajax({
			url: base_url+"tickets/consulta_ajax",
			type: "POST",
			...
			success:function(respuesta){
				...
				for (var i = registros.length - 1; i >= 0; i--) {
					...
					html+=""+
						"<td class='text-right'>"+
                        "<button type='button' class='btn btn-icon-toggle btn-atender' value="+registros[i]["folio"]+" data-toggle='modal' data-status="+registros[i]["status"]+" data-target='#ModalAtender'>"+
                        "<i class='fa fa-list-alt' data-toggle='tooltip' data-placement='top' data-original-title='Atender'></i>"+
                        "</button>"+
						"<button type='button' class='btn btn-icon-toggle btn-delete' data-toggle='tooltip' data-placement='top' value="+registros[i]["id"]+" data-original-title='Borar'>"+
